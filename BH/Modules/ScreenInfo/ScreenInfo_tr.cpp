...
void ScreenInfo::OnGameJoin() {
	...
	if (bFailedToWrite) {
		...
		for (int i = 0; i < 5; i++) {
			PrintText(Red, "文件 \"%s\" 被其它程序鎖定! 上一次遊戲統計不能保存!", ReplaceAutomapTokens(szSavePath));
		}
	}
	...
	*/
	...
	if (runname.length() > 0) {
		//mp
		PrintText(Orange, "%d 次總遊戲數。", nTotalGames);
		PrintText(Orange, "%d 次 \"%s\" 遊戲數。", runcounter[runname], runname.c_str());
		//PrintText(Orange, "%d \"%s\" games played total.", runs[runname], runname.c_str());
	}
	else {
		//sp
		PrintText(Orange, "%d 次總遊戲數。", nTotalGames);
		PrintText(Orange, "%d 次單人遊戲數。", runcounter[runname]);
...
void ScreenInfo::drawExperienceInfo() {
	...
	//sprintf_s(sExp, "等級：%00d，經驗：%00.2f%% (%s%00.2f%%) [%s%.2f%s/s]", cLevel, pExp, expGainPct >= 0 ? "+" : "", expGainPct, expPerSecond >= 0 ? "+" : "", expPerSecond, unit);
...
void ScreenInfo::OnAutomapDraw() {
	...
	// 		CHAR temp[255] = "";
	// 		//sprintf_s(temp, sizeof(temp), "回城捲軸只剩下%d個了!", dTPs);
	// 		//PrintText(Red, temp);
	// 		dTPsCheck = dTPs;
	// 	}
	// }
	...
	// 		CHAR temp[255] = "";
	// 		//sprintf_s(temp, sizeof(temp), "鑑定捲軸只剩下%d個了!", dIDs);
	// 		//PrintText(Red, temp);
	// 		dIDsCheck = dIDs;
	// 	}
	// }
	...
	for (vector<string>::iterator it = automapInfo.begin(); it < automapInfo.end(); it++) {
		...
		if (key.length() > 0) {
			...
			else if (key.find("鑑定") != string::npos) {
