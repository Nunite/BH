...
void Item::DrawSettings() {
	...
	settingsTab = new UITab("物品顯示", BH::settingsUI);
	unsigned int x = 8;
	unsigned int y = 7;

	// Settings
	new Drawing::Texthook(settingsTab, x, (y), "設置");
	y += 15;

	new Checkhook(settingsTab, x, y, &Toggles["Always Show Items"].state, "永久顯示物品");
	new Keyhook(settingsTab, GameSettings::KeyHookOffset, y + 2, &Toggles["Always Show Items"].toggle, "");
	y += 15;

	new Checkhook(settingsTab, x, y, &Toggles["Always Show Item Stat Ranges"].state, "顯示物品變量");
	new Keyhook(settingsTab, GameSettings::KeyHookOffset, y + 2, &Toggles["Always Show Item Stat Ranges"].toggle, "");
	y += 15;

	new Checkhook(settingsTab, x, y, &Toggles["Show iLvl"].state, "顯示物品等級");
	new Keyhook(settingsTab, GameSettings::KeyHookOffset, y + 2, &Toggles["Show iLvl"].toggle, "");
	y += 20;

	// Display Style
	new Drawing::Texthook(settingsTab, x, (y), "顯示樣式 (僅不啟用loot過濾時生效)");
	y += 15;

	Checkhook* etheral = new Checkhook(settingsTab, x, y, &Toggles["Show Ethereal"].state, "顯示ETH(無形)");
	new Keyhook(settingsTab, GameSettings::KeyHookOffset, y + 2, &Toggles["Show Ethereal"].toggle, "");
	y += 15;

	Checkhook* sockets = new Checkhook(settingsTab, x, y, &Toggles["Show Sockets"].state, "顯示孔數");
	new Keyhook(settingsTab, GameSettings::KeyHookOffset, y + 2, &Toggles["Show Sockets"].toggle, "");
	y += 15;

	Checkhook* runes = new Checkhook(settingsTab, x, y, &Toggles["Show Rune Numbers"].state, "顯示符文編號#");
	new Keyhook(settingsTab, GameSettings::KeyHookOffset, y + 2, &Toggles["Show Rune Numbers"].toggle, "");
	y += 15;

	Checkhook* alt = new Checkhook(settingsTab, x, y, &Toggles["Alt Item Style"].state, "Alt物品樣式");
	...
	Checkhook* shorten = new Checkhook(settingsTab, x, y, &Toggles["Shorten Item Names"].state, "物品短名稱");
	new Keyhook(settingsTab, GameSettings::KeyHookOffset, y + 2, &Toggles["Shorten Item Names"].toggle, "");
	y += 20;

	// Loot Filter
	new Drawing::Texthook(settingsTab, x, (y), "Loot過濾器");

	y += 15;
	new Checkhook(settingsTab, x, y, &Toggles["Advanced Item Display"].state, "開啟Loot過濾");
	...
	new Checkhook(settingsTab, x, y, &Toggles["Item Detailed Notifications"].state, "物品明細提示");
	new Keyhook(settingsTab, GameSettings::KeyHookOffset, y + 2, &Toggles["Item Detailed Notifications"].toggle, "");
	y += 15;

	new Checkhook(settingsTab, x, y, &Toggles["Verbose Notifications"].state, "提示更加豐富");
	new Keyhook(settingsTab, GameSettings::KeyHookOffset, y + 2, &Toggles["Verbose Notifications"].toggle, "");
	y += 15;

	colored_text = new Texthook(settingsTab, x, y + 2, "過濾等級:");
	...
	if (ItemFilterNames.size() == 0) {
		...
		ItemFilterNames.push_back(string("0 - 顯示所有物品"));
		ItemFilterNames.push_back("1 - 標準");
	}
...
void Item::ReplaceItemFilters(vector<string> itemFilterNames) {
	...
	for (auto it = settingsTab->Hooks.begin(); it != settingsTab->Hooks.end(); it++) {
		...
		if (dropDown != NULL) {
			if (dropDown->GetOptions()[0] == "0 - 顯示所有物品") {
