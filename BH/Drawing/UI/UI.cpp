...
void UI::OnDraw() {
	if (IsMinimized()) {
		...
		Texthook::Draw(GetX() + 4, GetY () + 3, false, 0, InTitle((*p_D2CLIENT_MouseX), (*p_D2CLIENT_MouseY))?Gold:White, GetName()+"(右鍵關閉)");
	...
	if (IsMinimized()) {
		...
		if (inPos /*&& GetAsyncKeyState(VK_CONTROL)*/) 
		{
			...
			else
			{
				...
				if(!up) {
					PrintText(7, "CTRL+左鍵 打開設置(Settings)" );
					PrintText(7, "Shift+左鍵 移動最小化按鈕" );
				}
			}
			return true;
		}
